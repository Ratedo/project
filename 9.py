...
class GameLogic(BaseGame):
    ...
    def generate_closeness_chart(self, save_path="."):
        ...
        plt.xticks(ticks=attempts)  # Устанавливаем целочисленные метки по оси X
        plt.gca().yaxis.set_major_locator(MaxNLocator(integer=True))  # Устанавливаем целочисленные метки по оси Y
...
class GameUI(ctk.CTk):
    ...
    def create_widgets(self):
        ...
        self.button_rules = ctk.CTkButton(self, text="Правила", font=ctk.CTkFont(size=16), command=self.show_rules)
    ...
    def show_rules(self):
        messagebox.showinfo(
            "Правила игры", 
            "Угадайте загаданное число от 1 до 100. Подсказки:\n"
            "🔥 'Очень горячо!' - разница с числом менее 5\n"
            "🌡️ 'Горячо.' - разница до 10\n"
            "☀️ 'Тепло.' - разница до 20\n"
            "❄️ 'Холодно.' - разница более 20\n"
        )
